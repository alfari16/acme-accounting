...
describe('ReportsService', () => {
  ...
  describe('startBackgroundGeneration', () => {
    ...
    beforeEach(() => {
      ...
      mockSetImmediate = jest.fn((fn: () => void) => {
        // Use setTimeout to simulate setImmediate behavior but make it testable
        setTimeout(fn, 0);
      });
      global.setImmediate =
        mockSetImmediate as unknown as typeof global.setImmediate;
    ...
    it('should start all three report generation tasks in background', async () => {
      ...
      const accountsSpy = jest
        .spyOn(service as any, 'accounts')
        .mockResolvedValue(1000);
      const yearlySpy = jest
        .spyOn(service as any, 'yearly')
    ...
    it('should handle individual report failures gracefully', async () => {
      ...
      const yearlySpy = jest
        .spyOn(service as any, 'yearly')
        .mockRejectedValue(new Error('Yearly generation failed'));
      const fsSpy = jest.spyOn(service as any, 'fs').mockResolvedValue(2000);
      ...
      const _jobIds = service.startBackgroundGeneration();
    ...
    it('should handle all report failures gracefully', async () => {
      ...
      const accountsSpy = jest
        .spyOn(service as any, 'accounts')
        .mockRejectedValue(new Error('Accounts failed'));
      const yearlySpy = jest
        .spyOn(service as any, 'yearly')
        .mockRejectedValue(new Error('Yearly failed'));
      const fsSpy = jest
        .spyOn(service as any, 'fs')
        .mockRejectedValue(new Error('FS failed'));
